...
open class AdHocController : WebServiceErrorController() {
    ...
    val account: IUser? get() = currentUser()
    ...
    protected fun testOwner(acc: IUser?) {
        if (acc == null || account != acc) {
            throw WebError.createException(HttpStatus.UNAUTHORIZED, "this is not yours, stop trying hacking!!!")
        }
    }

    protected fun <T> createErrorsSet(key: String, status: String, errors: Set<ConstraintViolation<T>>): ValidationErrors {
        return errorConversion.convertErrorsToJsonSet(422, key, status, errors)
    }
